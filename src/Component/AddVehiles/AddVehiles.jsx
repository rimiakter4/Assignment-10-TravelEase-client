...
const AddVehiles = () => {
  const { loading, setLoading, user } = useContext(Authcontext);
  const navigate = useNavigate(); // navigate use করব
  ...
  const handleAddVehicle = (e) => {
    ...
    // POST to same collection used in MyVehicles / VehiclesDetails
    fetch("http://localhost:3000/vehicles", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(newvehicle),
      ...
      .then((data) => {
        ...
        if (data.insertedId) {
          ...
          // Submit করার সাথে সাথে MyVehicles page এ navigate করে instant show
          navigate("/myvehicles");
        } else {
          toast.error("Failed to add vehicle.");
        }
